...
@section('content-header')
    <h1>
        {{ trans('ventas::ventas.title.edit venta') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.ventas.venta.index') }}">{{ trans('ventas::ventas.title.ventas') }}</a></li>
        <li class="active">{{ trans('ventas::ventas.title.edit venta') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.ventas.venta.update', $venta->id], 'method' => 'put']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('ventas::admin.ventas.partials.edit-fields', ['lang' => $locale])
                        </div>
                    @endforeach
